...
// Custom serialization functions to handle BigInt values
const customStorage = {
  getItem: (name: string) => {
    ...
    try {
      ...
      if (parsed.state) {
        if (parsed.state.totalValueLocked) {
          parsed.state.totalValueLocked = BigInt(parsed.state.totalValueLocked)
        }
        if (parsed.state.totalBorrowed) {
          parsed.state.totalBorrowed = BigInt(parsed.state.totalBorrowed)
        }
        if (parsed.state.vaults) {
          parsed.state.vaults = parsed.state.vaults.map((vault: any) => ({
            ...vault,
            btcCollateral: BigInt(vault.btcCollateral),
            usdtBorrowed: BigInt(vault.usdtBorrowed),
            accruedInterest: BigInt(vault.accruedInterest),
            createdAt: new Date(vault.createdAt),
            liquidatedAt: vault.liquidatedAt ? new Date(vault.liquidatedAt) : undefined,
            closedAt: vault.closedAt ? new Date(vault.closedAt) : undefined,
            lastInterestUpdate: new Date(vault.lastInterestUpdate),
          }))
        }
      }
      return parsed
  ...
  setItem: (name: string, value: any) => {
    try {
      ...
      const serializable = {
        ...
        state: {
          ...value.state,
          totalValueLocked: value.state.totalValueLocked.toString(),
          totalBorrowed: value.state.totalBorrowed.toString(),
          vaults: value.state.vaults.map((vault: Vault) => ({
            ...vault,
            btcCollateral: vault.btcCollateral.toString(),
            usdtBorrowed: vault.usdtBorrowed.toString(),
          ...
          vaults: state.vaults.map((vault) => {
            ...
            return {
              ...
              ltv:
                newBorrowed > 0
                  ? Math.round(
                      (Number(newBorrowed) / 1000000 / ((Number(vault.btcCollateral) * state.btcPrice) / 100000000)) *
                        100,
                    )
                  : 0,
            }
